ServerEvents.recipes(event => {
    event.recipes.modern_industrialization.centrifuge(16, 200)
        .itemIn("1x #c:foods/meat")
        .fluidOut("modern_industrialization:blood", 100)

    event.recipes.modern_industrialization.centrifuge(32, 400)
        .itemIn("6x roll_mod:rutile_iron")
        .itemOut("4x roll_mod:rutile_dust")
        .itemOut("2x #c:dusts/iron")
        .itemOut("1x #c:dusts/iron", 0.5)

    event.recipes.modern_industrialization.centrifuge(16, 200)
        .fluidIn("modern_industrialization:salt_water", 2000)
        .fluidOut("minecraft:water", 1000)
        .itemOut("2x #c:dusts/salt")

    event.recipes.modern_industrialization.centrifuge(32, 800)
        .itemIn("4x ad_astra:moon_sand")
        .itemOut("1x #c:dusts/silicon", 0.25)
        .itemOut("2x #c:dusts/aluminum", 0.1)
        .itemOut("1x #c:dusts/iron", 0.1)
        .fluidOut("modern_industrialization:helium_3", 20)

    event.recipes.modern_industrialization.centrifuge(32, 800)
        .fluidIn("modern_industrialization:aqua_regia", 4000)
        .itemIn("12x roll_mod:platinum_group_sludge")
        .itemOut("3x roll_mod:raw_platinum_dust")
        .itemOut("3x roll_mod:raw_palladium_dust")
        .itemOut("2x roll_mod:inert_metal_mixture")
        .itemOut("1x roll_mod:rarest_metal_dust")

    event.recipes.modern_industrialization.centrifuge(32, 400)
        .itemIn("5x roll_mod:iridium_metal_residue")
        .itemOut("4x roll_mod:iridium_chloride")

    event.recipes.modern_industrialization.centrifuge(64, 200)
        .fluidIn("modern_industrialization:diluted_agar_solution", 8000)
        .itemOut("1x roll_mod:agar_gel")

    event.recipes.modern_industrialization.centrifuge(32, 200)
        .itemIn("1x roll_mod:raw_latex")
        .itemOut("3x roll_mod:raw_rubber")
        .itemOut("2x #c:dusts/wood_pulp")
        .fluidOut("modern_industrialization:glue", 200)

    event.recipes.modern_industrialization.centrifuge(32, 100)
        .itemIn("2x roll_mod:cinnabar_dust")
        .itemOut("1x #c:dusts/sulfur")
        .fluidOut("modern_industrialization:mercury", 1000)

    event.recipes.modern_industrialization.centrifuge(32, 100)
        .itemIn("8x roll_mod:tetrahedrite_dust")
        .itemOut("3x #c:dusts/copper")
        .itemOut("3x #c:dusts/sulfur")
        .itemOut("1x #c:dusts/antimony")
        .itemOut("1x #c:dusts/iron")

    event.recipes.modern_industrialization.centrifuge(128, 100)
        .itemIn("6x roll_mod:pitchblende_dust")
        .fluidOut("modern_industrialization:radon", 150)
        .fluidOut("modern_industrialization:helium", 100)
        .itemOut("3x roll_mod:uraninite_dust")
        .itemOut("1x roll_mod:thorium_dust")
        .itemOut("1x #c:dusts/lead")

    event.recipes.modern_industrialization.centrifuge(16, 800)
        .fluidIn("modern_industrialization:uranium_hexafluoride", 1000)
        .fluidOut("modern_industrialization:depleted_uranium_hexafluoride", 800)
        .fluidOut("modern_industrialization:enriched_uranium_hexafluoride", 200)
        .fluidOut("modern_industrialization:radon", 50)
        .fluidOut("modern_industrialization:helium", 50)

    event.recipes.modern_industrialization.centrifuge(16, 100)
        .fluidIn("modern_industrialization:depleted_uranium_hexafluoride", 1000)
        .fluidOut("modern_industrialization:fluorine", 6000)
        .itemOut("1x #c:dusts/uranium_238")

    event.recipes.modern_industrialization.centrifuge(16, 100)
        .fluidIn("modern_industrialization:enriched_uranium_hexafluoride", 1000)
        .fluidOut("modern_industrialization:fluorine", 6000)
        .itemOut("1x #c:dusts/uranium_235")

    event.recipes.modern_industrialization.centrifuge(8, 200)
        .itemIn("1x roll_mod:sulfur_berry")
        .fluidOut("modern_industrialization:sulfuric_acid", 100)

    event.recipes.modern_industrialization.centrifuge(128, 1200)
        .itemIn("32x roll_mod:stone_dust")
        .itemOut("8x roll_mod:impure_quartz_dust")
        .itemOut("8x roll_mod:biotite_dust")
        .itemOut("8x roll_mod:metal_mixture_dust")
        .itemOut("4x roll_mod:marble_dust")
        .itemOut("4x roll_mod:calcium_carbonate")

    event.recipes.modern_industrialization.centrifuge(128, 1200)
        .itemIn("24x roll_mod:metal_mixture_dust")
        .itemOut("7x roll_mod:impure_yellow_limonite_dust")
        .itemOut("6x roll_mod:impure_malachite_dust")
        .itemOut("3x roll_mod:impure_magnetite_dust")
        .itemOut("3x roll_mod:impure_pentlandite_dust")
        .itemOut("3x roll_mod:impure_cassiterite_dust")
        .itemOut("2x roll_mod:impure_galena_dust")

    event.recipes.modern_industrialization.centrifuge(128, 1000)
        .itemIn("8x roll_mod:marble_dust")
        .itemOut("1x #c:dusts/magnesium")
        .itemOut("5x roll_mod:calcium_carbonate")

    event.recipes.modern_industrialization.centrifuge(128, 1200)
        .itemIn("48x roll_mod:deepslate_dust")
        .itemOut("10x roll_mod:soap_stone_dust")
        .itemOut("10x roll_mod:impure_lepidolite_dust")
        .itemOut("10x roll_mod:impure_quartz_dust")
        .itemOut("9x roll_mod:impure_redstone_dust")
        .itemOut("5x roll_mod:impure_bort_dust")
        .itemOut("4x roll_mod:basalt_dust")

    event.recipes.modern_industrialization.centrifuge(128, 500)
        .itemIn("16x roll_mod:basalt_dust")
        .itemOut("8x roll_mod:flint_dust")
        .itemOut("4x roll_mod:calcium_carbonate")
        .itemOut("2x roll_mod:olivine_dust")
        .itemOut("2x #c:dusts/coal")

    event.recipes.modern_industrialization.centrifuge(128, 1200)
        .itemIn("32x roll_mod:netherrack_dust")
        .itemOut("9x roll_mod:impure_topaz_dust")
        .itemOut("5x roll_mod:impure_sulfur_dust")
        .itemOut("5x roll_mod:impure_tetrahedrite_dust")
        .itemOut("5x roll_mod:impure_stibnite_dust")
        .itemOut("5x roll_mod:impure_cinnabar_dust")
        .itemOut("3x roll_mod:impure_pyrite_dust")

    event.recipes.modern_industrialization.centrifuge(32, 200)
        .itemIn("8x roll_mod:bort_dust")
        .itemOut("3x #c:dusts/diamond")
        .itemOut("3x roll_mod:deepslate_dust")

    event.recipes.modern_industrialization.centrifuge(128, 500)
        .itemIn("20x roll_mod:end_stone_dust")
        .itemOut("5x #c:sand")
        .itemOut("3x roll_mod:tungstate_dust")
        .itemOut("2x roll_mod:scheelite_dust")
        .itemOut("1x #c:dusts/glowstone")
        .itemOut("1x roll_mod:sheldonite_dust")
        .itemOut("1x roll_mod:monazite_dust")
        .fluidOut("modern_industrialization:helium", 250)

    event.recipes.modern_industrialization.centrifuge(128, 500)
        .itemIn("20x roll_mod:moon_stone_dust")
        .itemOut("5x ad_astra:moon_sand")
        .itemOut("4x roll_mod:ilmenite_dust")
        .itemOut("4x roll_mod:magnetite_dust")
        .itemOut("2x roll_mod:bauxite_dust")
        .itemOut("2x roll_mod:vanadium_magnetite_dust")
        .itemOut("2x roll_mod:chromite_dust")
        .fluidOut("modern_industrialization:helium_3", 250)
})
